...
! ---------------------------------------------------------------      
     ...
     5 ,theta,TolG, TolF,epsG,epsF,TrialSTRESSHat(3,3)
...
!     parameters for the material
      ...
      DOUBLE PRECISION Eh,nuh,Gh,Gv,gamma,ETRANS(3,3,3,3),psi
      DOUBLE PRECISION ROT(3,3),ETRANS2(3,3,3,3),pruef(3,3,3,3)
      DOUBLE PRECISION dep,deq,ps,pm,pss,pmm,denom,rrm,aa,bb,c,Spur
      DOUBLE PRECISION dfds(3,3),dfdh(3,3),dfdep(3,3),aux(3,3),
     1 Hplast(3,3,3,3),DEPS0(3,3),Depshat(3,3),a2,b2
      DOUBLE PRECISION DSTRANV(6),That(3,3),MM(3,3),set(3,3)
...
!     other material properties: alpha,beta...	
      ...
      Eh    =  PROPS(20)!young modulus
      nuh   =  PROPS(21)!poisson modulus horizontal
      Gv    =  PROPS(22)!shear modulus
      Gh    =  PROPS(23)!shear modulus 
      psi	  =  PROPS(24)!bedding orientation
	  
	  props2=props
	  if (nprops.gt.16) then
	     props2(3)=props(17)
		 props2(4)=props(18)
		 props2(5)=props(19)
	  endif
...
!      CALL Transversal(E,nu,Eh,nuh,Gv,Gh,gamma,ETRANS,ROT,psi)
...
!            calling the derivative evaluation
     ...
     &            PROPS,NPROPS,param,derve,dervp,dervh)
             call DerivF(TrialSTRESS,xx,NTENS,
...
!            initializing
             ...
             do i=1,6
                 ...
                 B(i+6)= abs(xx(13))*dervh(i)-xx(i+6) 
             enddo
             B(13)=-TrialF
...
!             write(6,*) TolG, TolF
             ...
             DEPL(2,1)= xx(10)
             DEPL(1,3)= xx(11)
             DEPL(3,1)=1.0d0*xx(11)
             DEPL(2,3)=1.0d0*xx(12)
             DEPL(3,2)=1.0d0*xx(12)
...
!     Will be needing dfds a lot...
         ...
         call Vectortomatrix(dervh,dfdh)		 
!     and dfdep
         call Vectortomatrix(dervp,dfdep) 
         aux = ELMOD.double.dfdh
         denom = dfds.double.aux 
!         write(6,*) denom  
         dum=dfdep.double.dfdh
         denom = denom - dum
...
!     other material properties: alpha,beta...	
      ...
      pt    = m*param(2)
	  stress0 = Stress
      call ROTATE(STRESS, PROPS , NPROPS)
      call pq(STRESS,p,q)
!     Lode angle 
...
C     Declarating other variables and constants    
     ...
     & STRESSDEV(3,3),temp2(3,3),Iunit(3,3,3,3),tempder(3,3),MM(3,3),
     & That(3,3),set(3,3),stress0(3,3),AN(3,3,3,3),a2,b2,c2
      double precision dfdps,dfdpm,ps,pm,ep,eq,Iun(3,3),strain(3,3),TINY
      DOUBLE PRECISION E,nu,m,rhos,ksis,rhom,ksim,pc0,aa,bb,c,Spur	  
C ---------------------------------------------------------------        
...
!      tempder = delta*derinv(1)/3.0+3./2.*derinv(2)*STRESSDEV  
      ...
      ! Stress projection - Anisotropy
      a2   =  PROPS(13)!rhom
      b2   =  PROPS(14)!ksim 
      c2   =  PROPS(15)!dummy variable to make units fit
      ...
      tempder =  double33x3333(tempder,AN)
...
C
  ...
  14  RETURN
      ...
      SUBROUTINE ROTATE(STRESS,PROPS,NPROPS)
C
      USE tensors
      implicit none
C --------------------------------------------------------------- 
      integer nprops,nrotate
      double precision stress(3,3),props(nprops),
     & That(3,3),a,b,c,MM(3,3),psi,Spur,stress0(3,3)
C --------------------------------------------------------------- 
C     Declarating other variables and constants    
C ---------------------------------------------------------------        
!     other material properties: alpha,beta...
      a   =  PROPS(13)!rhom 
      b   =  PROPS(14)!ksim 
      c   =  PROPS(15)!dummy variable to make units fit
      psi =  PROPS(24)*3.1416/180.
!     M is the tensorial product of the unit vector with itself (cos(psi) sin(psi)sin(?) sin(psi)cos(?) ) 
!     Taking ? equal to zero to simplify (0 sin(psi) cos(psi))... 
      nrotate   =  1.0
      MM = 0.0
!      MM(nrotate,nrotate) = PROPS(16)
      MM(3,3) = 0.
      MM(2,2) = sin(psi)**2
      MM(2,1) = sin(psi)*cos(psi)
      MM(1,2) = MM(2,3)
      MM(1,1) = cos(psi)**2
      Spur = tr(MM*stress)
      THat = ((a+c-2.0*b)*(Spur))*MM+c*stress+
     & (b-c)*(MM*stress+stress*MM)
...
! ---------------------------------------------------------------           
      ...
      SUBROUTINE ROTATE2(STRESS,PROPS,NPROPS)
C
      USE tensors
      implicit none
C --------------------------------------------------------------- 
      integer nprops,nrotate
      double precision stress(3,3),props(nprops),
     & That(3,3),a,b,c,MM(3,3),Spur,stress0(3,3)
C --------------------------------------------------------------- 
C     Declarating other vaiables and constants    
C ---------------------------------------------------------------        
!     other material properties: alpha,beta...
...
! ---------------------------------------------------------------           
      ...
      SUBROUTINE Transversal(E,nu,Eh,nuh,Gv,Gh,gamma,ETRANS2,
     1 R,psi)
	implicit none
      DOUBLE PRECISION gamma,E,nu,Eh,nuh,Gv,Gh,ETRANS(3,3,3,3)
      DOUBLE PRECISION R(3,3),psi, ETRANS2(3,3,3,3)
      INTEGER	i1,i2,i3,i4,j1,j2,j3,j4
      
      gamma = 1/(1-nuh**2-2*nu*nu-2*nuh*nu*nu)
      ETRANS = 0.0d0
      ETRANS(1,1,1,1) = gamma*Eh*(1-nu*nu)
      ETRANS(2,2,2,2) = gamma*Eh*(1-nu*nu)
      ETRANS(3,3,3,3) = gamma*E*(1-nuh**2)
      ETRANS(1,1,2,2) = gamma*Eh*(nuh+nu*nu)
      ETRANS(1,1,3,3) = gamma*Eh*(nu+nuh*nu)
      ETRANS(2,2,3,3) = gamma*Eh*(nu+nuh*nu)
      ETRANS(3,3,1,1) = ETRANS(1,1,3,3)
      ETRANS(2,2,1,1) = ETRANS(1,1,2,2)
      ETRANS(3,3,2,2) = ETRANS(2,2,3,3) 
      ETRANS(1,2,1,2) = Gh
      ETRANS(1,2,2,1) = Gh
      ETRANS(2,1,1,2) = Gh
      ETRANS(2,1,2,1) = Gh
      ...
      do 10 j3=1,3
      do 10 j4=1,3
  10  ETRANS2(i1,i2,i3,i4) = ETRANS2(i1,i2,i3,i4)+
     1   				  R(i1,j1)*R(i2,j2)*R(i3,j3)*
     2                  R(i4,j4)*ETRANS(j1,j2,j3,j4)
